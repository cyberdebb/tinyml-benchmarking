...
// Note: Just __ARM_FEATURE_DSP is checked to collect all intrinsics from the above mentioned groups
    ...
    // GCC added the ACLE DSP/SIMD32 intrinsics to <arm_acle.h> in GCC 10. Older GCC toolchains (e.g. the
    // default GCC 7.2.1 shipped by PlatformIO's ststm32 platform) lack them, which results in implicit
    // declarations and undefined references at link time. Provide inline assembly fallbacks for those.
    #if defined(__GNUC__) && !defined(__clang__) && !defined(__ARMCC_VERSION) && (__GNUC__ < 10)
...
{
    __ASM volatile("smlald %Q0, %R0, %1, %2" : "+r"(acc) : "r"(op1), "r"(op2));
    return (acc);
}
...
__STATIC_FORCEINLINE int32_t __sadd16(int32_t op1, int32_t op2)
{
    int32_t result;
